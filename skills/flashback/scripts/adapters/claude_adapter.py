#!/usr/bin/env python3
"""Claude runtime adapter for Flashback."""
...
class ClaudeAdapter:
    ...
    def _latest_claude_memory_candidate(self) -> Path | None:
        projects_dir = Path("~/.claude/projects").expanduser()
        if not projects_dir.exists():
            return None

        candidates = list(projects_dir.glob("*/memory/MEMORY.md"))
        if not candidates:
            return None
    ...
    def discover_memory(self, repo_root: Path, memory_dir_arg: str | None, dry_run: bool) -> tuple[Path | None, list[str], list[str]]:
        ...
        latest_claude = self._latest_claude_memory_candidate()
        if latest_claude is not None:
            candidates.append(latest_claude.resolve())
