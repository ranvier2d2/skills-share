...
- Browser proof state, especially when evidence is partial: `complete`, `partial_visual_evidence`, `http_only`, `screenshot_failed`, `http_failed`, or `no_visual_evidence`.
...
Do not use a narrow check for a broad claim: HTTP reachability proves the route responds, validators prove structural use of the target system, screenshots prove rendered evidence exists, and manual visual inspection or clean browser proof is needed before claiming visual acceptance.
