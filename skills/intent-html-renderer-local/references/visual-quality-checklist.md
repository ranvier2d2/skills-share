...
- Read `proofState` and each screenshot's `captureState`; do not treat `HTTP 200`, non-empty PNG files, or a timed-out Chrome process as equivalent to complete browser proof.
- If `proofState` is `partial_visual_evidence`, inspect the written screenshots manually when possible, fix any visible clipping/overflow, and report that automated browser proof was partial unless a clean rerun succeeds.
