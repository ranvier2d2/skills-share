...
### Electron Backend Discovery and Diagnostics

Some Electron apps expose their local backend URL through preload or another renderer bridge. If that exists, use it. It is often the most reliable way to inspect the exact backend instance owned by the launched Electron process.

Example for apps that expose a websocket URL through preload:

```javascript
var electronWsUrl = await appWindow.evaluate(
  () => window.desktopBridge?.getWsUrl?.() ?? null
);

var electronHttpBase = electronWsUrl
  ? electronWsUrl
      .replace(/^wss:/, "https:")
      .replace(/^ws:/, "http:")
      .replace(/\/\?token=.*$/, "")
...
console.log({ electronWsUrl, electronHttpBase });
```

Practical guidance:

- Use `appWindow.evaluate(...)` to inspect renderer state, DOM state, or preload-exposed bridges.
- Prefer the outer `js_repl` Node context for backend HTTP calls once you have the backend URL. That keeps diagnostics outside the renderer and avoids custom-scheme restrictions.
- If the backend URL includes a token query parameter, the same token may also be required for backend HTTP or websocket calls. Reuse it rather than guessing a second auth path.
- If renderer-side `fetch(...)` fails from `appWindow.evaluate(...)` but the app itself is otherwise healthy, suspect the renderer origin or scheme before assuming the backend is down.

Example backend HTTP call from the outer `js_repl` Node context after discovery:

```javascript
var response = await fetch(`${electronHttpBase}/api/health`);
console.log(response.status, await response.text());
```

For backend websocket inspection, the outer `js_repl` environment may not always have the most convenient websocket helpers or module resolution. If you need a direct backend probe and `js_repl` is awkward, a short one-off `node` shell probe is acceptable.
...
## Presenting screenshots in Codex desktop

If you are reporting local screenshot artifacts back to the user in the Codex desktop app, embed them with Markdown image syntax and an absolute filesystem path:

```md
![Mobile cockpit](/absolute/path/to/output/playwright/mobile-terminal.png)
```

Do not use a normal Markdown link when you want the image to appear inline:

```md
[Mobile cockpit](/absolute/path/to/output/playwright/mobile-terminal.png)
```

That produces only a clickable link.

Rules:

- Use absolute local paths for inline images.
- Prefer inline images for visual QA findings, screenshot comparisons, and signoff evidence.
- Use normal links only when the artifact should be referenced but not visually embedded.
- Empirical media behavior in Codex desktop:
  - `.gif` renders inline correctly with Markdown image syntax.
  - `.webm` should be shared as a normal link, not an inline Markdown image.

Examples:

```md
![Flow demo](/absolute/path/to/demo.gif)
[Open flow video](/absolute/path/to/demo.webm)
```
...
- `TypeError: Failed to fetch` from `appWindow.evaluate(...)` in Electron even though the app launched: if the renderer is running on a custom origin such as `t3://` or another app-specific scheme, do not treat that alone as backend failure. Discover the backend URL through preload or app bridges if available, then perform backend HTTP calls from the outer `js_repl` Node context instead of the renderer.
...
- Backend state updates are visible through direct HTTP or websocket probes but the Electron UI still does not refresh: that may be a renderer subscription or re-render bug in the target app rather than a Playwright or Electron-launch issue. Confirm backend state first, then debug the app's renderer sync path separately.
